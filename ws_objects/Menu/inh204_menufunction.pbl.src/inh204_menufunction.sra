$PBExportHeader$inh204_menufunction.sra
$PBExportComments$Generated Application Object
forward
global type inh204_menufunction from application
end type
...
string  gs_WinName[] = {"w_inh204w01_menufunction"}
//END_GLOBALVAR
end variables

global type inh204_menufunction from application
string appname = "inh204_menufunction"
event ue_cotopen ( string as_commandline )
end type
global inh204_menufunction inh204_menufunction
...
//		 Close(lw_Window[i])
// OpenSheet(lw_Window[i], ls_WinName[i], w_mdi, 1, Original!)
		if lower( ls_WinName[i]) = 'w_inh204w01_menufunction' then
			OpenSheet(w_inh204w01_menufunction, w_mdi, 1, Original!)
			lw_Window[i] = w_inh204w01_menufunction
			lw_Window[i].TriggerEvent("ue_RunAll")
		 Close(lw_Window[i])
		end if
	next
...
on inh204_menufunction.create
appname="inh204_menufunction"
...
on inh204_menufunction.destroy
destroy(sqlca)
destroy(sqlda)
destroy(sqlsa)
destroy(error)
